...
extension AddDishViewController: UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if let dish = AvailableDish.shared.getDish(index: indexPath.row) {
            ...
            OrderManager.shared.addDishToOrder(dish.nombre, OrderManager.shared.currentOrder)
            //Si no fuera navigation controller
            //self.dismiss(animated: true)
            self.navigationController?.popViewController(animated: true)
            print(OrderManager.shared.currentOrder)
